...
def test_sample_populator_deprecation():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert "deprecated and will be replaced with a router block" in str(w[0].message)

    # Check that generate raises NotImplementedError
    test_data = Dataset.from_dict({"route": ["test"]})
    with pytest.raises(NotImplementedError, match="deprecated and will be replaced with a router block"):
