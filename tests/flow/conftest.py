...
def sample_metadata():
    ...
    return FlowMetadata(
        ...
        recommended_models=[
            ...
            ModelOption(name="fallback-model", compatibility=ModelCompatibility.COMPATIBLE)
        ],
        tags=["test", "example"],
        estimated_cost="low",
...
class MockBlock(BaseBlock):
    ...
    def __init__(self, block_name="test_block", input_cols=None, output_cols=None, **kwargs):
        super().__init__(
    ...
    def __call__(self, dataset, **kwargs):
        ...
        if isinstance(self.output_cols, list):
            for col in self.output_cols:
                data[col] = [f"{self.block_name}_{col}_{i}" for i in range(len(dataset))]
        else:
            data[self.output_cols] = [f"{self.block_name}_{self.output_cols}_{i}" for i in range(len(dataset))]
